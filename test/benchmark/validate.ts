...
const main = async (): Promise<void> => {
  const trials: Array<number | null> = await Promise.all(
    new Array(100).fill(0).map(async () => {
      const counter = { value: 0 };
      try {
        const success = await ChatGptFunctionCaller.test({
          config: {
        ...
        return success !== false ? counter.value : null;
      } catch {
        return null;
      }
    }),
  ...
  console.log(trials);
  const successRate =
    trials.filter((trial) => trial === 1).length / trials.length;
