...
const main = async (): Promise<void> => {
  for (const title of await ShoppingSalePrompt.documents()) {
    ...
    try {
        ...
        GeminiFunctionCaller.test({
          ...
          handleCompletion: async (input) => {
            await fs.promises.writeFile(
              `${TestGlobal.ROOT}/examples/function-calling/arguments/gemini.${title}.input.json`,
      ...
      await fs.promises.writeFile(
        `${TestGlobal.ROOT}/examples/function-calling/arguments/gemini.${title}.error.json`,
