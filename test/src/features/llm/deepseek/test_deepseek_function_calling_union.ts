...
export const test_deepseek_function_calling_union = () =>
  DeepSeekFunctionCaller.test({
    ...
    handleParameters: async (parameters) => {
      if (process.argv.includes("--file"))
        await fs.promises.writeFile(
          `${TestGlobal.ROOT}/examples/function-calling/schemas/deepseek.union.schema.json`,
    ...
    handleCompletion: async (input) => {
      ...
      if (process.argv.includes("--file"))
        await fs.promises.writeFile(
          `${TestGlobal.ROOT}/examples/function-calling/arguments/deepseek.union.input.json`,
