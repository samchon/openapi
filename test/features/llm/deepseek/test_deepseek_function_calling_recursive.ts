...
export const test_deepseek_function_calling_recursive = () =>
  DeepSeekFunctionCaller.test({
    ...
    handleParameters: async (parameters) => {
      if (process.argv.includes("--file"))
        await fs.promises.writeFile(
          `${TestGlobal.ROOT}/examples/function-calling/schemas/deepseek.recursive.schema.json`,
    ...
    handleCompletion: async (input) => {
      ...
      if (process.argv.includes("--file"))
        await fs.promises.writeFile(
          `${TestGlobal.ROOT}/examples/function-calling/arguments/deepseek.recursive.input.json`,
