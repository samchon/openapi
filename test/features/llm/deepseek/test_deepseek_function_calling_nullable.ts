...
export const test_deepseek_function_calling_nullable = (): Promise<void> =>
  DeepSeekFunctionCaller.test({
    ...
    handleParameters: async (parameters) => {
      if (process.argv.includes("--file"))
        await fs.promises.writeFile(
          `${TestGlobal.ROOT}/examples/function-calling/schemas/deepseek.nullable.schema.json`,
    ...
    handleCompletion: async (input) => {
      ...
      if (process.argv.includes("--file"))
        await fs.promises.writeFile(
          `${TestGlobal.ROOT}/examples/function-calling/arguments/deepseek.nullable.input.json`,
